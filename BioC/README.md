...
- Kept the ICA call based on the `ica` package:
  - original package imports `ica`
  - `NAMESPACE` uses `importFrom(ica, ica)`
  - `performICA()` calls `ica(t(Y0), com)`
...
The original package already uses the `ica` package:

```text
Imports: ica
NAMESPACE: importFrom(ica, ica)
performICA(): ica(t(Y0), com)
```

`loadings` does not provide `ica()`, but that is not a problem for this specific
function because the intended backend appears to be `ica::ica()`.

Current behavior in `BioC/MS2DecR-bioc`:
...
- keep using the `ica` package,
- compare results against an alternative implementation such as `fastICA`, or
- document why `ica::ica()` is the preferred backend.

Since the package describes the workflow as ICA-based, keeping an actual ICA
